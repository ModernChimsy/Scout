// lib/view/home_view/controller/today_event_controller.dart
...
class TodayEventController extends GetxController {
  ...
  Future<void> getInitialTodayEvents() async {
    currentPage.value = 1;
    hasMore.value = true;
    eventList.clear();
  ...
  Future<void> fetchTodayEvents() async {
    ...
    try {
      final api = ApiUrl.todayEvent(page: currentPage.value, limit: Constant.perPage);

      log.d("🧩 Fetching Today Events from: $api");
      ...
      if (responseBody != null) {
        ...
        log.e('Failed to fetch today events, response body is null.');
      }
    } catch (e) {
      log.e("🧩 Error occurred during today event fetch: $e");
    } finally {
      isLoading(false);
      isPaginating(false);
    }
  ...
  getToday() => getInitialTodayEvents();
}
